...
import { 
  User, Patient, OPDVisit, IPDAdmission, Bed, Medicine, 
  LabTest, LabOrder, RadiologyOrder, OTSchedule, HRClock, 
  BillingTransaction, HospitalSettings, AuditLog, UserRole, WardType 
} from './src/types';
...
const PORT = 3000;
const DB_FILE = path.join(__dirname, 'data', 'hospital_db.json');
...
let db: {
  ...
  settings: {
    ...
    whatsappTemplate: "Assalam-o-Alaikum {patientName},\nYour payment of Rs. {amount} has been processed at {hospitalName}.\nMRN: {mrn}\nTransaction/Invoice ID: {txnId}\nThank you for choosing Mayo Trust.",
    smsTemplate: "Dear Patron, your OPD token is #{token} for Ward {department}. Track live status on our app. Mayo Trust.",
    logoText: "Mayo Care"
  },
  auditLogs: []
};
...
function loadDb() {
  if (fs.existsSync(DB_FILE)) {
    try {
      ...
      if (!db.patients || db.patients.length < 10 || !db.users || db.users.length < 6) {
        console.log("Upgrading loaded database structure to match fresh Pakistani high-fidelity demo profiles...");
        seedDb();
      }
    } catch (error) {
      console.error("Database Parse Failed, seeding new state:", error);
      seedDb();
    }
  } else {
    seedDb();
...
function seedDb() {
  console.log("Seeding Database with clean Pakistani administrative profiles...");

  // Registered Clinicians and Administrators
  db.users = [
    { id: "EMP-4040", name: "Dr. Mushtaq Khan", role: "DOCTOR", department: "CARDIOLOGY", phone: "0300-1112233", email: "doc.mushtaq@mayotrust.pk", shift: "MORNING", status: "ACTIVE", salary: 280000, cnic: "35201-8812345-9" },
    { id: "EMP-4041", name: "Dr. Ayesha Naeem", role: "DOCTOR", department: "MATERNITY", phone: "0321-4445566", email: "ayesha.n@mayotrust.pk", shift: "MORNING", status: "ACTIVE", salary: 260000, cnic: "35202-1234567-2" },
    { id: "EMP-4042", name: "Dr. Haris Jameel", role: "DOCTOR", department: "GENERAL_OPD", phone: "0333-5556677", email: "h.jameel@mayotrust.pk", shift: "EVENING", status: "ACTIVE", salary: 190000, cnic: "34101-7765432-1" },
    { id: "EMP-1010", name: "Dr. Asif Mayo", role: "SUPER_ADMIN", department: "ADMINISTRATION", phone: "0300-8889900", email: "asif.admin@mayotrust.pk", shift: "MORNING", status: "ACTIVE", salary: 350000, cnic: "35201-1111111-1" },
    { id: "EMP-2020", name: "Dr. Sadia Malik", role: "DOCTOR", department: "GENERAL_OPD", phone: "0345-2233445", email: "sadia.doctor@mayotrust.pk", shift: "NIGHT", status: "ACTIVE", salary: 185000, cnic: "35202-9988776-6" },
    { id: "EMP-3030", name: "Qasim Siddiqui", role: "CASHIER", department: "ACCOUNTS", phone: "0322-1234567", email: "qasim.cash@mayotrust.pk", shift: "MORNING", status: "ACTIVE", salary: 85000, cnic: "35201-3456789-3" },
    { id: "EMP-5050", name: "Yousuf Raza", role: "LAB_TECH", department: "DIAGNOSTICS", phone: "0311-9988776", email: "yousuf.lab@mayotrust.pk", shift: "EVENING", status: "ACTIVE", salary: 110000, cnic: "35102-1212121-7" },
    { id: "EMP-6060", name: "Sonia Bibi", role: "RECEPTIONIST", department: "FRONT_DESK", phone: "0300-7654321", email: "sonia.front@mayotrust.pk", shift: "MORNING", status: "ACTIVE", salary: 65000, cnic: "35202-4545454-4" },
  ];
  ...
  db.patients = [
    ...
    },
    {
      ...
      photoUrl: "https://images.unsplash.com/photo-1438761681033-6461ffad8d80?q=80&w=120&auto=format&fit=crop",
    ...
    },
    {
      ...
      photoUrl: "https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?q=80&w=120&auto=format&fit=crop",
    ...
    },
    {
      ...
      photoUrl: "https://images.unsplash.com/photo-1519085360753-af0119f7cbe7?q=80&w=120&auto=format&fit=crop",
    ...
    },
    {
      ...
      photoUrl: "https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?q=80&w=120&auto=format&fit=crop",
  ...
  db.ipdAdmissions = [
    {
      ...
      status: "ADMITTED",
      nurseNotes: [
        { time: "2026-06-08T22:00:00Z", note: "Patient vitals stable. BP 130/85, Pulse 82. Intravenous fluids running smoothly.", nurseName: "Nurse Fatima Batool" }
      ],
      doctorVisits: [
        { time: "2026-06-08T10:00:00Z", notes: "Condition improved post angiography. Keep monitored for 24h.", doctorId: "EMP-4040", doctorName: "Dr. Mushtaq Khan", charge: 2500 }
      ],
  ...
  db.labOrders = [
    {
      ...
      isCritical: true,
      notes: "Severe dyslipidemia detected. Requires aggressive statin therapy.",
  ...
  db.radiologyOrders = [
    {
      ...
      status: "COMPLETED",
      findings: "Single live intrauterine pregnancy of 14 weeks. Fetal heart rate is 144 bpm. Placenta is anterior and clear.",
  ...
  db.otSchedules = [
    {
      ...
      preOpChecklist: { consentSigned: true, pacCompleted: true, fastingOk: true, markedSite: true },
      charges: 185000,
      status: "SCHEDULED",
  ...
  db.opdVisits = [
    {
      ...
      status: "COMPLETED",
      vitals: { bp: "115/75", temp: 37.0, weight: 64, pulse: 78, recordedAt: "2026-06-09T09:00:00Z" },
      symptoms: "Routine second-trimester review. Mild fatigue.",
      diagnosis: "Normal Gestational Progress with minor iron deficiency anemia.",
      prescription: [
        { medicineId: "MED-003", name: "Fefol Vit capsule (Iron + Folic Acid)", dosage: "1-0-0", frequency: "After Breakfast", duration: "30 Days", quantity: 30 }
      ],
      billingStatus: "PAID",
      billAmount: 1500
    }
  ...
  db.billingTransactions = [
    {
      ...
      items: [
        { description: "OPD Consultation Gynaecology Token #4", qty: 1, price: 1500 }
      ]
    },
    {
      id: "INV-2026-0002",
  ...
  db.hrClocks = [
    ...
    { id: "CLK-102", staffId: "EMP-1010", staffName: "Irfan Qureshi", role: "SUPER_ADMIN", date: "09-06-2026", clockIn: "07:55", status: "PRESENT" },
    { id: "CLK-103", staffId: "EMP-2020", staffName: "Nurse Fatima Batool", role: "NURSE", date: "09-06-2026", clockIn: "19:50", status: "PRESENT" }
  ];
  ...
  db.auditLogs = [
    {
      ...
      action: "BOOT_SYSTEM",
      ipAddress: "127.0.0.1",
      details: "Hospital Management Database core models synchronized and seeded successfully."
    }
  ];
...
// Audit log helper
function addAuditLog(staffId: string, action: string, details: string, req: express.Request) {
  const staff = db.users.find(u => u.id === staffId) || { name: "System", role: "SUPER_ADMIN" as UserRole };
  const log: AuditLog = {
...
app.post('/api/auth/login', async (req, res) => {
  ...
  if (!employeeId || !password) {
    return res.status(400).json({ error: "Please provide both Employee ID and password." });
  }

  try {
    const authResult = await AuthService.authenticateUser(employeeId, password, req.ip);
    ...
    addAuditLog(authResult.user.id, "LOGIN_SUCCESS", `Authorized as ${authResult.user.role} with JWT cookies setup.`, req);
...
app.post('/api/auth/refresh-token', async (req, res) => {
  ...
  try {
    ...
    const userObj = db.users.find(u => u.id === payload.employeeId || u.id === payload.userId) || {
      id: payload.userId,
    ...
    const accessToken = jwt.sign(
      { userId: userObj.id, employeeId: userObj.id, role: userObj.role, name: userObj.name },
...
app.get('/api/auth/me', (req, res) => {
  ...
  try {
    ...
    return res.status(401).json({ error: 'Access token invalidated or expired.' });
  }
});
...
app.post('/api/auth/change-password', async (req, res) => {
  ...
  if (!newPassword || newPassword.length < 6) {
    return res.status(400).json({ error: 'Password must be at least 6 characters with secure length.' });
  }
  ...
  try {
    ...
    if (userToUpdate) {
      ...
      saveDb();
      addAuditLog(decoded.userId, "PASSWORD_MUTATION", `Re-keyed authorization password securely.`, req);
...
app.post('/api/whatsapp/send-receipt', async (req, res) => {
  const { phone, patientName, billNo, services, total, paid, balance } = req.body;
  try {
...
app.get('/api/admin/backup', (req, res) => {
  ...
  res.setHeader('Content-Disposition', `attachment; filename=MayoHMS_Backup_${Date.now()}.json`);
...
app.post('/api/audit', (req, res) => {
  ...
  addAuditLog(staffId || "SYSTEM", action || "SYSTEM_EVENT", details || "", req);
...
app.post('/api/patients', (req, res) => {
  const { 
    name, fullNameUrdu, fatherName, dateOfBirth, age, gender, bloodGroup, 
    address, phone, whatsapp, email, city, province, 
    emergencyContactName, emergencyContactPhone, emergencyRelation, 
    insuranceProvider, insurancePolicyNumber, insuranceCompany, photoUrl, staffId, cnic
  } = req.body;

  const resolvedCnic = cnic || req.body.cnic;
  if (!name || !age || !gender || !bloodGroup || !phone || !resolvedCnic) {
    return res.status(400).json({ error: "Required fields are missing. Make sure to complete demographics." });
  }
  ...
  const newPatient: any = {
    ...
    photoUrl: photoUrl || `https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?q=80&w=120&auto=format&fit=crop`,
  ...
  addAuditLog(staffId || "SYSTEM", "PATIENT_REGISTER", `Registered new patient ${name} successfully with MRN ${mrn}`, req);
...
app.put('/api/patients/:mrn', (req, res) => {
  ...
  const { 
    ...
    address, phone, whatsapp, email, city, province, 
    emergencyContactName, emergencyContactPhone, emergencyRelation, 
    insuranceProvider, insurancePolicyNumber, insuranceCompany, photoUrl 
  } = req.body;

  const updatedPatient = {
    ...existing,
    name: name || existing.name,
    fullNameUrdu: fullNameUrdu !== undefined ? fullNameUrdu : (existing as any).fullNameUrdu,
    fatherName: fatherName !== undefined ? fatherName : (existing as any).fatherName,
    dateOfBirth: dateOfBirth !== undefined ? dateOfBirth : (existing as any).dateOfBirth,
    ...
    emergencyRelation: emergencyRelation !== undefined ? emergencyRelation : (existing as any).emergencyRelation,
    insuranceProvider: insuranceProvider || existing.insuranceProvider,
    insurancePolicyNumber: insurancePolicyNumber || existing.insurancePolicyNumber,
    insuranceCompany: insuranceCompany !== undefined ? insuranceCompany : (existing as any).insuranceCompany,
  ...
  addAuditLog(req.body.staffId || "SYSTEM", "PATIENT_UPDATE", `Updated patient coordinates for ${mrn}`, req);
...
app.delete('/api/patients/:mrn', (req, res) => {
  ...
  addAuditLog("SYSTEM", "PATIENT_ARCHIVE", `Soft deleted / Archived patient MRN ${mrn}`, req);
...
app.post('/api/opd/visits', (req, res) => {
  const { patientMrn, doctorId, symptoms, symptomsText, staffId, billAmount, specialty } = req.body;
  const patient = db.patients.find(p => p.mrn === patientMrn);
  const doctor = db.users.find(u => u.id === doctorId);

  if (!patient || !doctor) {
    return res.status(400).json({ error: "Invalid patient MRN or Doctor ID selected." });
  }

  const tokenNumber = db.opdVisits.filter(v => 
    v.doctorId === doctorId && 
  ...
  addAuditLog(staffId || "SYSTEM", "OPD_SCHEDULE", `Issued Token #${tokenNumber} for patient ${patient.name} under ${doctor.name}`, req);
...
app.put('/api/opd/visits/:id', (req, res) => {
  const { id } = req.params;
  const { status, bp, temp, weight, pulse, diagnosis, prescription, referralDepartment, staffId } = req.body;
  ...
  saveDb();
  addAuditLog(staffId || "SYSTEM", "OPD_UPDATE_CLINICAL", `Updated health chart of ${visit.patientName} (Token #${visit.tokenNumber})`, req);
  res.json(visit);
});
...
app.post('/api/ipd/admissions', (req, res) => {
  ...
  if (!patient || !bed || !doctor) {
    return res.status(400).json({ error: "Invalid patient, bed code, or clinician ID entered." });
  }

  if (bed.status !== 'AVAILABLE') {
    return res.status(400).json({ error: "The chosen bed is currently occupied or undergoing maintenance." });
  }
  ...
  addAuditLog(staffId || "SYSTEM", "IPD_ADMISSION", `Admitted ${patient.name} to Bed ${bedId} in ward ${bed.ward}`, req);
...
app.put('/api/ipd/admissions/:id', (req, res) => {
  const { id } = req.params;
  const { action, note, diagnosis, treatmentGiven, followUpInstructions, dischargeMedication, doctorId, notes, charge, staffId } = req.body;
  ...
  if (action === 'NURSE_NOTE') {
    ...
    const doc = db.users.find(u => u.id === doctorId) || { name: "Consultant Physician" };
    const visitCharge = Number(charge) || 1500;
    admission.doctorVisits.push({
      time: new Date().toISOString(),
      notes: notes || "Systemic review completed. Patient shows active recovery.",
    ...
    admission.dischargeSummary = {
      ...
      treatmentGiven: treatmentGiven || "Double stent therapeutic angioplasty, active beta-blocker routines.",
      followUpInstructions: followUpInstructions || "Complete cardiac rest for 14 days. Avoid high sodium diets.",
      dischargeMedication: dischargeMedication || []
    };

    // Calculate final billing breakdown
    const diffTime = Math.abs(new Date(admission.dischargedAt).getTime() - new Date(admission.admittedAt).getTime());
  ...
  saveDb();
  addAuditLog(staffId || "SYSTEM", "IPD_MODIFY", `Updated admission chart for ${admission.patientName} (Action: ${action})`, req);
  res.json(admission);
});
...
app.post('/api/billing/transactions', (req, res) => {
  const { patientMrn, module, subtotal, discountApprovedAmount, discountReason, taxAmount, netBill, insuranceClaimProvider, insuranceClaimAmount, amountPaidByPatient, paymentMethod, items, staffId } = req.body;

  if (!patientMrn || !subtotal || !items || items.length === 0) {
    return res.status(400).json({ error: "Required transaction detail parameters are missing." });
  }

  const patient = db.patients.find(p => p.mrn === patientMrn) || { name: "Walk-in Patron" };
  const staff = db.users.find(u => u.id === staffId) || { name: "Accounts Counter Cashier" };
  ...
  const newTxn: BillingTransaction = {
    ...
    paymentStatus: (Number(amountPaidByPatient) >= Number(netBill)) ? "PAID" : "PARTIAL",
  ...
  if (module === 'OPD') {
    const matchingOPD = db.opdVisits.find(v => v.patientMrn === patientMrn && v.billingStatus === 'PENDING');
    if (matchingOPD) matchingOPD.billingStatus = 'PAID';
  }

  saveDb();
  addAuditLog(staffId || "SYSTEM", "GENERATE_INVOICE", `Generated transaction invoice ${invoiceId} for ${patient.name} - net Rs. ${netBill}`, req);
...
app.post('/api/billing/whatsapp', (req, res) => {
  ...
  text = text.replace('{patientName}', patientName || "Valued Patron")
  ...
  console.log(`[SIMULATED WHATSAPP DISPATCH] To Phone: ${phone || "Patient Mobile"} -> Text:\n${text}`);
  
  addAuditLog("SYSTEM", "WHATSAPP_RECEIPT", `Dispatched billing WhatsApp receipt for invoice ${txnId} to patient ${patientName}.`, req);
  res.json({ success: true, message: "WhatsApp receipt template dispatched successfully!", text });
});
...
app.post('/api/lab/orders', (req, res) => {
  ...
  const doctor = db.users.find(u => u.id === doctorId) || { name: "OPD Specialist" };

  if (!patient || !test) {
    return res.status(400).json({ error: "Missing or invalid patient MRN or test code." });
  }
  ...
  addAuditLog(staffId || "SYSTEM", "ORDER_LAB_TEST", `Placed lab diagnostical order for ${patient.name} - Test: ${test.name}`, req);
...
app.put('/api/lab/orders/:id', (req, res) => {
  ...
  const tech = db.users.find(u => u.id === staffId) || { name: "Lead Lab Diagnostician" };
  ...
  saveDb();
  addAuditLog(staffId || "SYSTEM", "LAB_UPDATE_REPORT", `Recorded diagnostical results for order card ${order.id}`, req);
...
app.post('/api/pharmacy/dispense', (req, res) => {
  ...
  if (medicine.stockCount < Number(qty)) {
    return res.status(400).json({ error: `Insufficient pharmacy inventory. Only ${medicine.stockCount} units available.` });
  }

  medicine.stockCount -= Number(qty);
  saveDb();

  addAuditLog(staffId || "SYSTEM", "DISPENSE_PHARMACY", `Dispensed ${qty} units of ${medicine.name} for Patient ${patientName || "Ad-Hoc"}`, req);
...
app.post('/api/pharmacy/purchase-order', (req, res) => {
  ...
  addAuditLog(staffId || "SYSTEM", "REPLENISH_PHARMACY", `Replenished medicine ${medicine.name} (+${addedQty} units)`, req);
...
app.post('/api/radiology/orders', (req, res) => {
  ...
  const patient = db.patients.find(p => p.mrn === patientMrn);
  const doctor = db.users.find(u => u.id === doctorId) || { name: "Referral Clinician" };

  if (!patient || !type) {
    return res.status(400).json({ error: "Missing patient MRN or radiology study category." });
  }
  ...
  addAuditLog(staffId || "SYSTEM", "ORDER_RADIOLOGY", `Created ${type} clinical scan request for ${patient.name}`, req);
...
app.put('/api/radiology/orders/:id', (req, res) => {
  ...
  order.findings = findings || "Anatomical elements fall within physiological parameters.";
  order.imageUrl = imageUrl || "https://images.unsplash.com/photo-1559757175-5700dde675bc?q=80&w=140&auto=format&fit=crop";
  order.completedAt = new Date().toISOString();

  saveDb();
  addAuditLog(staffId || "SYSTEM", "RADIOLOGY_REPORT_ENTRY", `Uploaded diagnosis findings for radiology scan ID ${order.id}`, req);
...
app.post('/api/ot/schedules', (req, res) => {
  const { patientMrn, surgeryName, scheduledAt, theaterNumber, team, charges, staffId } = req.body;
  const patient = db.patients.find(p => p.mrn === patientMrn);

  if (!patient || !surgeryName || !scheduledAt) {
    return res.status(400).json({ error: "Incomplete OT surgical booking parameters." });
  }
  ...
  const newSchedule: OTSchedule = {
    ...
    preOpChecklist: { consentSigned: false, pacCompleted: false, fastingOk: false, markedSite: false },
  ...
  addAuditLog(staffId || "SYSTEM", "OT_SCHEDULE", `Scheduled ${surgeryName} surgery for patient ${patient.name}`, req);
...
app.put('/api/ot/schedules/:id', (req, res) => {
  ...
  saveDb();
  addAuditLog(staffId || "SYSTEM", "OT_UPDATE_SURGERY", `Updated clinical checkpoints/status of surgery card ${schedule.id}`, req);
...
app.post('/api/hr/clocks', (req, res) => {
  ...
  const currentDate = date || new Date().toLocaleDateString('en-GB').replace(/\//g, '-'); // DD-MM-YYYY
  const currentTime = clockIn || new Date().toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' });

  if (action === 'CLOCK_IN') {
    const existingClock = db.hrClocks.find(c => c.staffId === staffId && c.date === currentDate);
    if (existingClock) {
    ...
    addAuditLog(staffId, "HR_CLOCK_IN", `Clocked in for shift at ${currentTime}`, req);
    return res.status(201).json(clock);
  } else if (action === 'CLOCK_OUT') {
    const clock = db.hrClocks.find(c => c.staffId === staffId && c.date === currentDate && !c.clockOut);
    if (!clock) {
      return res.status(400).json({ error: "Punch-in entry not found or already clocked out for today." });
    }

    clock.clockOut = clockOut || new Date().toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' });
    saveDb();

    addAuditLog(staffId, "HR_CLOCK_OUT", `Clocked out of shift at ${clock.clockOut}`, req);
...
app.post('/api/hr/staff', (req, res) => {
  const { name, role, department, phone, email, shift, cnic, salary, staffId } = req.body;

  if (!name || !role || !department || !cnic) {
    return res.status(400).json({ error: "Primary parameters (Name, Role, Dept, CNIC) are required." });
  }
  ...
  addAuditLog(staffId || "SYSTEM", "HR_ADD_STAFF", `Enrolled staff member ${name} as ${role} inside administrative database. ID: ${employeeId}`, req);
...
app.put('/api/settings', (req, res) => {
  const { hospitalName, hospitalAddress, phone, taxRatePercent, whatsappTemplate, smsTemplate, logoText, staffId } = req.body;
  ...
  if (taxRatePercent !== undefined) db.settings.taxRatePercent = Number(taxRatePercent);
  ...
  saveDb();
  addAuditLog(staffId || "SYSTEM", "UPDATE_SETTINGS", `Reconfigured general system/hospital configurations`, req);
...
app.get('/api/dashboard/analytics', (req, res) => {
  ...
  const moduleRevenue = db.billingTransactions.reduce((acc, txn) => {
  ...
  const outstandingInsurance = db.billingTransactions
    .filter(txn => txn.insuranceClaimProvider !== 'NONE' && txn.paymentStatus !== 'PAID')
    .reduce((val, txn) => val + txn.insuranceClaimAmount, 0);

  // Doctor Patient Load count
  const doctorLoad = db.opdVisits.reduce((acc, visit) => {
    acc[visit.doctorName] = (acc[visit.doctorName] || 0) + 1;
    return acc;
  }, {} as Record<string, number>);

  // Expiry notifications countdown (pharmacy)
  const lowStockMedicines = db.medicines.filter(m => m.stockCount <= m.minStockLevel).length;

  res.json({
    totalPatients: db.patients.length,
    opdQueueToday: db.opdVisits.length,
    activeAdmissions: db.ipdAdmissions.filter(a => a.status === 'ADMITTED').length,
    occupancyRate,
    moduleRevenue,
...
async function startServer() {
  if (process.env.NODE_ENV !== "production") {
    console.log("Starting Hot-reloading Vite dev server inside Express pipeline...");
